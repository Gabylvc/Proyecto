...
const int ledCargado = 19, ledMedio = 18, ledBajo = 5, ledEncendido = 15;  // LEDs de bateria
float angMin = 9999, angMax = -9999, radio = 0.15;
...
int batteryStatus(float v_bat) {
  if (v_bat >= 4.0) return 3;
  else if (v_bat >= 3.7) return 2;
  else if (v_bat >= 3.5) return 1;
  else return 0;
}
...
void loop() {
  ...
  if (SerialBT.available()) {
    ...
    dato.trim();  // Elimina espacios o retornos
    float nuevoRadio = dato.toFloat();
    if (nuevoRadio > 0) { 
      radio = nuevoRadio;
      Serial.print("Nuevo radio recibido: ");
      Serial.print(radio);
      Serial.println(" m");
    }
  }
